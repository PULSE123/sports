...
def ach_detail(request, Post_id):
    try:
        post= achPost.objects.get(pk=Post_id)
    except Post.DoesNotExist:
        raise Http404("Post Does Not Exist")
    return render(request,'tw/achieve.html',{'post':post})    
...
def ath(request):
    ...
    return render(request, 'tw/adetail.html', context)   
